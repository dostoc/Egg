...
select nombre, sal_emp, comision_emp, sum(sal_emp + comision_emp + 500) as 'Total' from empleados
	where id_depto = 3000
	Group by nombre, sal_emp, comision_emp
	order by nombre;
...
select nombre, sal_emp, comision_emp, sum(sal_emp + comision_emp) as 'total' from empleados
...
SELECT nombre, sal_emp as 'total' from empleados
	where sal_emp>1000 and comision_emp = 0;
...
select nombre, sal_emp, comision_emp from empleados
	where comision_emp*0.7< sal_emp;
...
select nombre, departamentos.nombre_depto from empleados inner join departamentos
	where empleados.id_depto = departamentos.id_depto
		and departamentos.nombre_depto in('Ventas','Investigación','Mantenimiento');
...
select nombre, departamentos.nombre_depto from empleados inner join departamentos
	where empleados.id_depto = departamentos.id_depto
		and not departamentos.nombre_depto in('Ventas','Investigación','Mantenimiento');
...
select departamentos.nombre_depto from departamentos inner join empleados
...
select departamentos.nombre_depto from departamentos left join empleados
	on departamentos.id_depto = empleados.id_depto
...
select nombre, sal_emp from empleados inner join departamentos
